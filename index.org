...
#+RESULTS:
:RESULTS:
: loading patient data: 100% 9/9 [00:06<00:00,  1.37it/s]
:
...
#+begin_src jupyter-python :session py :exports both :async yes :eval never-export
  ...
  fd_img = utils.window_image(fd_data.reshape((fd_data.shape[0], fd_data.shape[1])) * 1000, ww, wl)
  ld_img = utils.window_image(ld_data * 1000, ww, wl)
  uld_fbp = utils.window_image(mayo.add_poisson_noise(fd_data.reshape((fd_data.shape[0], fd_data.shape[1])), N0=30000) * 1000, ww, wl)
  figures = [('Normal Dose', axs[0, 0], fd_img),
             ('Low Dose', axs[0, 1], ld_img),
             ('Ultra Low Dose (Simple)', axs[1, 0], mayo.add_simple_noise(fd_img, peak=20)),
             ('Ultra Low Dose (FBP)', axs[1, 1], uld_fbp),
             ]
...
#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/0a0e71f109e481f7a90acd825f603d7296276284.png]]
: -0.44244975502892425 vs 0.04130843026318939
:END:
* DDPM
