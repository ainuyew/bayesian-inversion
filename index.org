...
#+begin_src jupyter-python :session py :exports both :async yes :eval never-export
  ...
  ddpm_state = utils.create_training_state(BATCH_SIZE, params_file=f'{PROJECT_DIR}/ddpm_params_{epoch}_{step}_{loss}.npy')
...
#+RESULTS:
:RESULTS:
: using parameters from epoch 9 with loss 0.14745
: 100% 200/200 [01:12<00:00,  2.75it/s]
:
#+attr_org: :width 534
[[file:./.ob-jupyter/6b3f6df30b49f3dd621897855d66e721ef554ccd.png]]
:END:
...
#+begin_src jupyter-python :session py :exports both :async yes :eval never-export
  ...
  cem_state = utils.create_training_state(BATCH_SIZE, params_file=f'{PROJECT_DIR}/cem_params_{epoch}_{step}_{loss}.npy')
  print(f'using parameters from epoch {epoch} with loss {loss}')
