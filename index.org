...
- summary on windowing CT scans https://kevalnagda.github.io/ct-windowing
*** DONE to investigate out of memory issues when working with larger images 512x512
*** DONE store samples in files (instead of in memory) to calculate mean and variance. HDF5 format https://www.pytables.org/FAQ.html or https://docs.h5py.org/en/stable/#:~:text=HDF5%20lets%20you%20store%20huge,and%20tagged%20however%20you%20want. we cannot load everything into memory.
** 2024-03-30
- Running CEM sampling on server for 1000 samples. This is based on 128x128 training data and conditioned on 36th image frame where the lesion was identified. Samples are recorded in HDF5. From which, we can calculate the mean/variance.
- Rewrote the CEM/DDPM training code to run on 512x512 training data. The training data will be loaded into memory (when needed) from HDF5 files instead of the entire data into memory. CEM training code ran for 2 epochs on M1 without out of memory errors. Will test on server when CEM sampling is done. Better resolution is expected from 512x512 data.
- Wrote code to generate CEM samples outside of org (jupyter construct). This code will be executed on server instead. Yet to be tested.
** 2024-03-31
- windowing of CT images is probably not neccessary for training. should test out preprocessing training data to be within -1000 HU to 1000 HU. Resulting sample will then simply be multiplied by 1000 and to window for display.
* Problem Statement
...
#+begin_src jupyter-python :session py :exports both :async yes :eval never-export
  ...
  test_data= utils.window_image(test_data, ww, wl, out_range=(-1., 1.))
  ...
  with h5py.File(f'{Path.home()}/Documents/data/mayo.hdf5', 'w') as hf:
    ...
    #hf["train"].resize((hf["train"].shape[0] + training_data.shape[0]), axis = 0)
    #hf["train"][-training_data.shape[0]:] = training_data

    #hf["test"].resize((hf["test"].shape[0] + test_data.shape[0]), axis = 0)
    #hf["test"][-test_data.shape[0]:] = test_data
...
#+RESULTS:
:RESULTS:
: loading patient data: 100% 9/9 [00:06<00:00,  1.38it/s]
:
: loading patient data: 100% 1/1 [00:00<00:00,  1.64it/s]
:
: done
:END:
...
#+begin_src jupyter-python :session py :exports both :async yes :eval never-export
  ...
  _ = fig.tight_layout()
  _ = axs[0, 0].set_title("full dose image")
  _ = axs[0, 0].imshow(utils.window_image(fd_data, 350, 50), cmap=plt.cm.Greys_r)
  _ = axs[0, 1].set_title("quarter dose image")
  _ = axs[0, 1].imshow(utils.window_image(ld_data, 350, 50), cmap=plt.cm.Greys_r)
  _ = axs[1, 0].set_title("CEM mean")
  _ = axs[1, 0].imshow(utils.window_image(sample_mean, 350, 50), cmap=plt.cm.Greys_r)
  _ = axs[1, 1].set_title("CEM variance")
  #_ = axs[1, 1].imshow(wd(sample_variance), **imkwargs)
  _ = axs[1, 1].imshow(utils.window_image(sample_variance, 350, 50))

  plt.show()
#+end_src

#+RESULTS:
[[file:./.ob-jupyter/2b1fc5abf5912a361a8667e5d9752bb60caa18b1.png]]
