...
#+begin_src jupyter-python :session py :exports both :async yes :eval never-export
  ...
  figures = [('Ground Truth', ax1, utils.window_image(fd_data * 1000 - 1000, ww, wl)),
             ('Low Dose Image', ax2, utils.window_image(ld_data * 1000 - 1000, ww, wl))]
...
#+RESULTS:
:RESULTS:
: loading patient data: 100% 9/9 [00:07<00:00,  1.22it/s]
:
: loading patient data: 100% 1/1 [00:00<00:00,  1.48it/s]
:
[[file:./.ob-jupyter/65d0e15acde4044765db32a059d11e6c094bd1b1.png]]
:END:
...
#+begin_src jupyter-python :session py :exports both :async yes :eval never-export
  ...
  fd_img = utils.window_image(fd_data.reshape((fd_data.shape[0], fd_data.shape[1])) * 1000 - 1000, ww, wl)
  ld_img = utils.window_image(ld_data * 1000 - 1000, ww, wl)
  uld_fbp = utils.window_image(mayo.add_noise(fd_data.reshape((fd_data.shape[0], fd_data.shape[1])), N0=30000) * 1000 - 1000, ww, wl)
  figures = [('Normal Dose', axs[0, 0], fd_img),
             ('Low Dose', axs[0, 1], ld_img),
             ('Image + Noise', axs[1, 0], utils.window_image(mayo.add_simple_noise(fd_data, peak=2) * 1000 - 1000, ww, wl)),
  ...
  print(f'{np.mean(uld_fbp - fd_img)} vs {np.mean(ld_img - fd_img)}')
#+end_src

#+RESULTS:
:RESULTS:
[[file:./.ob-jupyter/b71baf084087f3e079a53fd1a7ec20a98b4faaec.png]]
: -0.3526370895365686 vs 0.04130843026318939
:END:
* DDPM
