...
#+begin_src jupyter-python :session py :exports both :async yes :eval never-export
  ...
  data = mayo.get_training_data(path)
  n = (len(data) // 10) * 9
  training_data = data[:n]
  test_data = data[n:]

  #h5f.create_dataset('X_train', data=orig_data, compression="gzip", chunks=True, maxshape=(None,x))
...
#+RESULTS:
: loading patient data: 100% 10/10 [00:54<00:00,  5.40s/it]
...
#+begin_src jupyter-python :session py :exports both :async yes :eval never-export
  ...
  ld_data_windowed = utils.window_image(ww, wl, out_range=(-1., 1.))
