...
*** TODO to investigate out of memory issues when working with larger images 512x512
*** TODO store samples in files (instead of in memory) to calculate mean and variance. HDF5 format https://www.pytables.org/FAQ.html or https://docs.h5py.org/en/stable/#:~:text=HDF5%20lets%20you%20store%20huge,and%20tagged%20however%20you%20want. we cannot load everything into memory.
* Problem Statement
...
#+begin_src jupyter-python :session py :exports both :async yes :eval never-export
  ...
  #with h5py.File('.\mayo.h5', 'a') as hf:
  with h5py.File('/Users/huiyuanchua/Documents/data/mayo.hdf5', 'w') as hf:
...
#+RESULTS:
:RESULTS:
: loading patient data: 100% 9/9 [00:19<00:00,  2.14s/it]
:
: loading patient data: 100% 1/1 [00:01<00:00,  1.93s/it]
:
: done
:END:
...
#+begin_src jupyter-python :session py :exports both :async yes :eval never-export
  ...
  with h5py.File('/Users/huiyuanchua/Documents/data/cem_samples.hdf5', 'w') as hf:
  ...
  ww = 350
  wl = 50
  wd = lambda x : utils.window_image(x, ww, wl)

  # rescale [-125HU, 225HU] to [-1, 1]
  ld_data_windowed = utils.window_image(ld_data, ww, wl, out_range=(-1., 1.))
  ...
  for i in tqdm(range(n_samples // BATCH_SIZE)):
    ...
    with h5py.File('/Users/huiyuanchua/Documents/data/cem_samples.hdf5', 'a') as hf:
      hf['samples'][(i * BATCH_SIZE):((i + 1) * BATCH_SIZE)] = x_0_tilde

  with h5py.File('/Users/huiyuanchua/Documents/data/cem_samples.hdf5', 'r') as hf:
    samples = hf['samples']
