...
#+RESULTS:
:RESULTS:
: loading patient data: 100% 9/9 [00:05<00:00,  1.58it/s]
:
: loading patient data: 100% 1/1 [00:00<00:00,  1.81it/s]
:
: done
:END:
...
:RESULTS:
: Platform 'METAL' is experimental and not all JAX functionality may be correctly supported!
: 2024-03-29 16:08:19.095068: W pjrt_plugin/src/mps_client.cc:563] WARNING: JAX Apple GPU support is experimental and not all JAX functionality is correctly supported!
: Metal device set to: Apple M1
: using parameters from epoch 68 with loss 0.00039
: loading patient data: 100% 10/10 [00:00<00:00, 58.49it/s]
:
:  17% 17/100 [5:28:27<26:45:03, 1160.28s/it]
:END:
:END:
...
#+begin_src jupyter-python :session py :exports both :async yes :eval never-export
  ...
  with h5py.File(f'{Path.home()}/Documents/data/cem_samples.hdf5', 'r') as hf:
    samples = hf['samples']
    sample_mean = np.mean(samples, axis=0)
    sample_variance = np.var(samples, axis=0)
...
#+begin_src jupyter-python :session py :exports both :async yes :eval never-export
  ...
  import h5py
  with h5py.File(f'{Path.home()}/Documents/data/mayo.hdf5', 'r') as hf:
      images = [utils.window_image(img[0], 350, 50) for img in hf['test'][36:52]]
...
#+RESULTS:
[[file:./.ob-jupyter/8f6594d4311b23397a4a2e373ea01d76ceb19063.png]]
